...
class BooleanQueryParser:
    ...
    def _find_next_literal(subquery : str, start_index : int) -> 'BooleanQueryParser._Literal':
        ...
        # Locate the phrase literal
        if subquery[start_index] == '"':
            start_index += 1
            
            # Locate the next double quote to find the end of phrase literal
            next_double_quote = subquery.find('"', start_index)
            if next_double_quote < 0:
                # No more literals in this subquery.
                length_out = sub_length - start_index
            else:
                length_out = next_double_quote - start_index+1

            # length_out = next_double_quote - start_index 
            return BooleanQueryParser._Literal(
                BooleanQueryParser._StringBounds(start_index, start_index + length_out),
        ...
        if subquery[start_index] == '[':
            ...
            #locate next )
            next_closing_bracket = subquery.find(']', start_index)
            if next_closing_bracket < 0:
                length_out = sub_length - start_index
            else:
                length_out = next_closing_bracket - start_index

            print('returning near literal')
            return BooleanQueryParser._Literal(
                BooleanQueryParser._StringBounds(start_index, start_index + length_out+1),
                NearLiteral(subquery[start_index:start_index + length_out])
            )
    ...
    def parse_query(self, query : str) -> QueryComponent:
        ...
        # print('subqueries = ', all_subqueries)
